...
export interface WSMessageListener {
  ...
  onError?: (context: ZWEB_WEBSOCKET_CONTEXT, ws: ZWEBWebsocket) => void
  onClosed?: (context: ZWEB_WEBSOCKET_CONTEXT, ws: ZWEBWebsocket) => void
}
...
export class ZWEBWebsocket {
  ...
  messageQueue: string[] = []
  status: ZWEB_WEBSOCKET_STATUS = ZWEB_WEBSOCKET_STATUS.INIT
  context: ZWEB_WEBSOCKET_CONTEXT = ZWEB_WEBSOCKET_CONTEXT.DASHBOARD
  listeners: WSMessageListener[] = []

  constructor(url: string, context: ZWEB_WEBSOCKET_CONTEXT) {
    this.url = url
    ...
    if (this.ws) {
      ...
      this.ws.onopen = () => {
        ...
        store.dispatch(
          configActions.updateWSStatusReducer({
            context: this.context,
            wsStatus: ZWEB_WEBSOCKET_STATUS.CONNECTED,
          }),
        )
        ...
        this.send(
          getTextMessagePayload(
            ...
            [
              {
                authToken: ZWEBBuilderStorage.getLocalStorage("token"),
              },
            ],
          ),
        )
