...
export const runActionWithExecutionResult = async (
  ...
  const isProductionMode = getIsZWEBProductMode(rootState)
  const {
  ...
  try {
    ...
    const zwebInnerTransformedResponse = transResponse(
      actionType,
      response,
      isClientS3,
    )
    ...
    let userTransformedData = runTransformer(
      transformer,
      zwebInnerTransformedResponse.data ?? "",
    )
