...
test.describe("MySQL create And delete", () => {
  ...
  test("create Resource", async ({ page }) => {
    ...
    await page.getByPlaceholder("Hostname").fill(process.env.ZWEB_MYSQL_HOST)
    await page.getByPlaceholder("3306").click()
    await page.getByPlaceholder("3306").fill(process.env.ZWEB_MYSQL_PORT)
    await page.getByPlaceholder("acme_production").click()
    await page
      .getByPlaceholder("acme_production")
      .fill(process.env.ZWEB_MYSQL_DATABASENAME)
    await page.getByPlaceholder("Username").click()
    await page
      .getByPlaceholder("Username")
      .fill(process.env.ZWEB_MYSQL_DATABASEUSERNAME)
    await page.getByPlaceholder("Password").click()
    await page
      .getByPlaceholder("Password")
      .fill(process.env.ZWEB_MYSQL_DATABASEPASSWORD)
