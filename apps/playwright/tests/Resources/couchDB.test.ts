...
test.describe("CouchDB create And delete", () => {
  ...
  test("create Resource", async ({ page }) => {
    ...
    await page.getByRole("textbox").nth(1).fill(process.env.ZWEB_COUCHDB_HOST)
    await page.getByPlaceholder("5984").click()
    await page.getByPlaceholder("5984").fill(process.env.ZWEB_COUCHDB_PORT)
    await page.getByRole("textbox").nth(3).click()
    await page
      .getByRole("textbox")
      .nth(3)
      .fill(process.env.ZWEB_COUCHDB_USERNAME)
    await page.locator('input[type="password"]').click()
    await page
      .locator('input[type="password"]')
      .fill(process.env.ZWEB_COUCHDB_PASSWORD)
